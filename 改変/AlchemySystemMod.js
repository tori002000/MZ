...
@min 0
@desc 経費に使う値
0で所持金　1以上を指定するとそのIDの変数

@param PriceUnit
@text 単位
@type string
@default Ｇ
@desc 合成シーンで表示される経費の単位
財布が変数の場合に有効

@param StartScript
@text 開始スクリプト
@desc 合成シーン開始時に実行されるスクリプト
@type multiline_string
@default 

@param EndScript
@text 終了スクリプト
@desc 合成シーン終了時に実行されるスクリプト
@type multiline_string
@default 
...
/*:ja
@target MV MZ
@plugindesc アイテム合成プラグイン v2.1.2 変数財布改変 by「」
@author うなぎおおとろ
@url https://raw.githubusercontent.com/unagiootoro/RPGMZ/master/AlchemySystem.js
...
改変ポイント
・経費を変数で支払えるように
・合成シーン開始時と終了時にスクリプトを実行できるように
利用例　「主人公（アクターID1）のMPを消費して合成」　
財布　：（変数ID）
単位　：MP
開始時：$gameVariables.setValue(変数ID,$gameActors.actor(1).mp);
終了時：$gameActors.actor(1).setMp($gameVariables.value(変数ID));
---------------------------------------------------------------------------
...
const AlchemySystemPluginName = document.currentScript ? decodeURIComponent(document.currentScript.src.match(/^.*\/(.+)\.js$/)[1]) : "AlchemySystemMod";
var AlchemySystem;
(function (AlchemySystem) {
    ...
    const Wallet = PP.Wallet;
    const PriceUnit = PP.PriceUnit;
    const StartScript = PP.StartScript;
    const EndScript = PP.EndScript;
    ...
    class AlchemyRecipe {
        ...
        maxMakeItemCountNoLimit() {
            let minCount;
            if (Wallet) {
                minCount = (this._price > 0 ? Math.floor($gameVariables.value(Wallet) / this._price) : MaxNumMakeItem);
            } else {
        ...
        makeItem(targetItemCount) {
            ...
            if (Wallet) {
                $gameVariables.setValue(Wallet, $gameVariables.value(Wallet) - this._price * targetItemCount);
            } else {
    ...
    class Window_RecipeDetail extends Window_Base {
        ...
        drawTotalPrice() {
            ...
            if (y < minY)
                y = minY;
            const currentUnit = PriceUnit;
            const width = this.innerWidth - this.itemPadding() * 2;
            const goldText = `${this._recipe.price() * this._numberWindow.number()}`;
            this.changeTextColor(this.systemColor());
            this.drawText(NeedPriceText, x, y, width, "left");
            this.resetTextColor();
            if ($gameVariables.value(Wallet)<this._recipe.price()) 
                this.changePaintOpacity(false);
            this.drawText(goldText, x, y, width - this.textWidth(currentUnit) - 6, "right");
            this.changePaintOpacity(true);
            this.changeTextColor(this.systemColor());
    ...
    function isAlchemySceneActive() {
        return SceneManager._scene instanceof Scene_Alchemy;
    }
    ...
    Window_Gold.prototype.drawCurrencyValue = function (value, unit, x, y, width) {
        if (isAlchemySceneActive() && Wallet) {
            const a = $gameVariables.value(Wallet);
            this.resetTextColor();
            this.drawText(a, x, y, width - this.textWidth(PriceUnit) - 6, 'right');
            this.changeTextColor(ColorManager.systemColor());
            this.drawText(PriceUnit, x + width - this.textWidth(PriceUnit), y, this.textWidth(PriceUnit), 'right');
        } else {
            // 通常の所持金表示
            _Window_Gold_drawCurrencyValue.call(this, value, unit, x, y, width);
        }
    };
